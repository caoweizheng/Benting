/**
 * const prefixCls = 'styles-31673575';
 * const images = '/static/images/stores';
 * @Author: Jack
 * @Date:   2018-01-03 16:04:58
 * @Last Modified by: czy0729
 * @Last Modified time: 2018-06-27 17:04:30
 * @Path btWap \stores\common.js
 */
'use strict';
...
class store {
    ...
    setState(data, key = defaultKey) {
        ...
        if (this.debug && isStateChange) {
            console.info(
                Utils.date('H:i:s', new Date().valueOf() / 1000),
                '|',
                key
            );
     ...
     */
    @action
    clearState(key = defaultState, data = {}) {
        extendObservable(this.state, {
            [key]: data
        });
    }
     ...
     * @version 180626 1.2 可以控制是否跳到最顶
     * @return {Void}
     */
    @action
    setRefresh(params, resetTop = true) {
        const _params = {
            [updateKey]: true,
            ...params
        };
    ...
    async fetchListThenSetState(
        ...
        //判断是否还有下一页
        if (!refresh && state._loaded && !state._hasMore) return;

        //加载过的数据会强制使用store里面保存的query，以使请求数据合并后不会错乱
        let _query;
        if (refresh || !state._loaded) {
            //首次请求
            _query = {
                _: {
                    page: 1,
        ...
        const _list = refresh ? data.list : [...list, ...data.list];
    ...
    async updateOneThenSetState(api, key = defaultKey, search = {}, searchKey) {
        ...
        if (!searchKey) searchKey = Object.keys(search)[0];

        //(重要) 假如列表对应这项不存在，不执行
        const findIndex = state.list.findIndex(
            item => item[searchKey] == search[searchKey]
        );

        if (!state._loaded || findIndex === -1) return;
