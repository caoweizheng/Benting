 ...
 * @Last Modified time: 2018-07-26 15:57:15
...
const doFetch = (fns, target) =>
  Promise.all(
    fns.map(item => target.fetch[Array.isArray(item) ? item[0] : item](true))
  );
      ...
      getChildContext() {
        ...
        if (typeof window !== 'undefined') {
          if (!window.Stores) {
            ...
            if (igoreFetch.length) doFetch(igoreFetch, this.$);
