/**
 * const prefixCls = 'styles-21007449';
 * const images = '/static/images/src/person/welfare/Meet';
 * @Author: Jack
 * @Date:   2018-01-09 16:44:23
 * @Last Modified by:   Jack
 * @Last Modified time: 2018-05-25 15:46:28
 * @Path btWap \src\person\welfare\Meet\_RankList.js
 */
'use strict';
...
const _RankList = (props, { $ }) => {
    ...
    return (
        <ListWrap className={classNames(prefixCls, className)} title="见面有礼">
            <p className="text-sm">当前等级：{Utils.getBTLevel(btlevel)}</p>
            <div className="mt-d">
                {lotteryGradeInfo.list.map((item, index) => (
                    <div
                        key={index}
                        className="item"
                        style={{
                        ...
                        onClick={() =>
                            Utils.router.push(
                                `/person/welfare/meet?id=${item.grade}`,
                                `/person/welfare/meet/${item.grade}`
                            )}
                    >
                        {item.state === 2 && (
                            <Flex
