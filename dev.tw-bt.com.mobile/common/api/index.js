/**
 * const prefixCls = 'styles-66130738';
 * const images = '/static/images/common/api';
 * @Author: Jack
 * @Date:   2018-01-03 16:05:05
 * @Last Modified by: czy0729
 * @Last Modified time: 2018-07-16 16:06:09
 * @Path btWap \common\api\index.js
 */
'use strict';
...
const apis = {
     ...
     */
    do_feed_back_add: '/adm/feedback/add',

    /**
     * 14.2 我的反馈列表
     * @version 170520 1.0
     */
    get_my_feed_back_list: '/adm/feedback/myFeedBack',
    /**
     * 14.3 更新意见反馈为已读
     * @version 170520 1.0
     * @param {int}       tbId    反馈id号，多个用,隔开；为空代表更新所有为已读
     */
    do_feed_back_set_read: '/adm/feedback/read',
     ...
     * @version 170526 1.0
     */
    get_integral_record_list: '/users/score/myscorelogs',
    /**
     * 19.2  我的等级信息  或 等级信息
     * @version 170526 1.0
     */
     ...
     */
    do_alipay_pay: 'https://lumen.nidosport.com/alipay',
     ...
     */
    'do_registration_submit-order': `${Const.__NEW_API__}/submitregistrationorder`,
     ...
     */
    'get_registration_record-detail': `!${Const.__NEW_API__}/getregistrationdetailbyid`,
     ...
     * @param {String} *code   验证码
     * @param {String} *openid 百度openid
     */
    'do_baidu_bind': '/baidu/bind'
};
...
const _fetch = async (api, query, isSubmitApi = false) => {
    ...
    const isUrl = api.indexOf('https://') !== -1;
    let response;
...
const P = async (api, query = {}, config = {}) => {
    const appTk = Utils.getQuery('tk');
    if (appTk) {
        const plantform = Utils.getQuery('plantform') || 'ios';

        G.setState(appTk, 'tk');
        G.setState(plantform, 'plantform');
    }
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if (Const.__DEV__ && Const.__SERVER__) {
                Utils.log('[api]', api, res.code);
            }
                    ...
                    if (isShowError && !_confirming) {
                        ...
                        Utils.onConfirm(
                            ...
                            () => {
                                ...
                                //Utils.router.replace('/person/fans_prove');
                                _confirming = false;
                            },
                            undefined,
                            () => {
                ...
                case 1004:
                    if (isShowError && !_confirming) {
                        ...
                        Utils.onConfirm(
                            '该操作需要绑定旺旺ID，前往绑定?',
                            () => {
                                Utils.router.push('/person/bind_ww');
                                _confirming = false;
                            },
                            undefined,
                            () => {
...
const PP = (api, query = {}, config = {}) => {
    return new Promise(async (resolve, reject) => {
        ...
        let apiAddress = apis[api].replace(/!/g, '');
...
const getRequestUrl = (api, query = {}) => {
    ...
    return `${isUrl ? '' : Const.__API__}${apiAddress}?tk=${tk}&${Utils.getQueryStr(query)}`;
};
