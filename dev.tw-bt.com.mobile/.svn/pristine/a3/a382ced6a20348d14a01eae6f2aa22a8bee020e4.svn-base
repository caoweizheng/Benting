 ...
 * v1.1
 * v2.0 180521 代码优化
 * v2.1 180529 添加[LiftCycle]storeInit，用于代替store里面的constructor
 * v2.2 180601 支持自定义服务器SSR需要请求的方法，配置为[fn, [...,] true]
 * const prefixCls = 'styles-15344402';
 * const images = '/static/images/decorator';
 * @Author: Jack
 * @Date:   2017-12-29 17:10:02
 * @Last Modified by: czy0729
 * @Last Modified time: 2018-07-24 13:53:21
 * @Path btWap \decorator\inject.all.js
 */
'use strict';
    ...
    ['static'].forEach(mode => {
        initFetch[mode] &&
            initFetch[mode].forEach(item => {
                ...
                if (
                    !Object.keys(gState).filter(item => item.indexOf('_') !== 0)
                        .length
                ) {
                    fns.push(fn);
                } else {
